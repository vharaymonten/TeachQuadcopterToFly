...
class DDPG():
    ...
    def __init__(self, task, state_size, action_size, action_low, action_high):
        self.task = task
        ...
        # Actor (Policy) Model
        self.actor_local = Actor(self.state_size, self.action_size, action_low, action_high)
        self.actor_target = Actor(self.state_size, self.action_size,  action_low, action_high)
        ...
        for episode in range(1, n_episodes+1):
            ...
            while True:
                ...
                if done:
                    ...
                    avg_rewards = np.mean(self.all_rewards)
                    self.training_results.append((episode, episode_reward, avg_rewards,\
                                                  self.task.sim.pose, self.task.sim.v, self.task.sim.angular_v))
                    '''
                    print("\r Episode : {}/{}  Reward : {:.4f} Avg Reward :{:.4f} time_steps : {}".format(episode,\
                                                                              n_episodes, episode_reward,\
                                                                             avg_rewards, t),end='')
                    '''
                    print("Episode : {}/{}  Reward : {:.4f} Avg Reward :{:.4f} time_steps : {}".format(episode,\
                                                                              n_episodes, episode_reward,\
                                                                             avg_rewards, t))
                    break;
